...
#taking input and giving response by predicting
...
def main():
    ...
    #Header
    st.header("Chat Smarter, Not Harder with ChitChat AI!🥸")
    st.info("Just ask me anything,I am your personal chitchat bot💻")
    ...
    if choice == "Home":
        ...
        counter += 1
         # Use Streamlit's form to group the input and button together
        with st.form(key=f'chat_form_{counter}',clear_on_submit=True,border=False ):
         ...
         # Create a 'Run' button inside the form
            run_button = st.form_submit_button("Run",icon="▶️")

        # Create a placeholder for the conversation display
        chat_display = st.container(border=False)
        ...
        if run_button or user_input:
            ...
            with chat_display:
                for user_msg, bot_msg in reversed(st.session_state.chat_history):
                    ...
                    st.write(f"**Chit-Chat AI:** {bot_msg}")
                    st.markdown("---")
